...
public class TTSReport extends Purchase{
	...
	@ExcelField(title="去哪报表实收价格", align=3, sort=50)
	...
	@ExcelField(title="实收价格含保险", align=3, sort=55)
	public Double getActualPriceAndInsurance(){
		if(StringUtils.isEmpty(super.getPassengerList().get(0).getInsureCount())){
			return  Double.valueOf(this.getActualPriceR());
		}
		return new BigDecimal(this.getActualPriceR()).add(new BigDecimal(Integer.parseInt(super.getPassengerList().get(0).getInsureCount())*15)).doubleValue();
	}
