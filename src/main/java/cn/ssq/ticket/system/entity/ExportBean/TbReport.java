...
public class TbReport extends Purchase{
	...
	/*@ExcelField(title="证件号", align=1, sort=23)
	...
	public Double getProfit(){
		...
		if(super.getPassengerNum()>1){
			return (new BigDecimal(getActualPriceR()).subtract(new BigDecimal(getActualPriceR()).multiply(new BigDecimal("0.015"))).subtract(new BigDecimal(getPayPurchPriceR())).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());

		}else{			
			return (new BigDecimal(getCustomerAmount()).subtract(new BigDecimal(getCustomerAmount()).multiply(new BigDecimal("0.015"))).subtract(new BigDecimal(getPayAmount())).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());
		}
	}
	...
	@ExcelField(title="起飞日期", align=1, sort=90)
	public String getFlightDate(){
		return super.getFlightDate();
	}

	@ExcelField(title="出票人员", align=1, sort=95)
	...
	@ExcelField(title="支付流水号            ", align=1, sort=100)
	public String getTradeNo() {
		return super.getTradeNo();
	}

	@ExcelField(title="实际支付价格", align=1, sort=105)
	...
	@ExcelField(title="报表类型", align=1, sort=106)
