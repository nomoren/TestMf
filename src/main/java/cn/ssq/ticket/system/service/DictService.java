...
import cn.ssq.ticket.system.mapper.DictMapper;
...
public class DictService extends ServiceImpl<DictMapper, Dict> {
	 ...
	 * @return
	 */
	@Cacheable(cacheNames="dict",key="#type+#name")
	public String getDictCode(String type,String name){
		String code=baseMapper.getDictCode(type, name);
		return code==null?name:code;
	}
	
	
	public String getDictCodeNoCache(String type,String name){
		String code=baseMapper.getDictCode(type, name);
		return StringUtils.isEmpty(code)?name:code;
	}
	 ...
	 * @return
	 */
	@Cacheable(cacheNames="dict",key="#type+#code")
	public String getDictName(String type,String code){
		String name=baseMapper.getDictName(type, code);
    ...
    @Cacheable(cacheNames="dict",key="#description")
    public List<String> getPrincipalAccount(String description,String code){
    	List<String> principalAccount = baseMapper.getPrincipalAccount(description, code);
    	if(principalAccount.size()<1){
    		principalAccount.add("admin");
    	}
    	return principalAccount;
    }
    
   @Cacheable(cacheNames="dict",key="#type")
   public List<String> getDictNameList(String type){
	   return baseMapper.getDictNameList(type);
   }
