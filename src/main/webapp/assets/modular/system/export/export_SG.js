layui.use(['layer', 'element', 'laydate', 'form'], function() {
	...
	//下载出票明细
	form.on('submit(exportSG)', function(data) {		
		var data = $('#printSG').serialize()
		var tokenValue=new Date().Format("yyyyMMddHHmmss");
		var downToken="down"+tokenValue;
		var url=Feng.ctxPath+'/export/exportPrintSG?'+data+"&downToken="+downToken+"&tokenValue="+tokenValue
