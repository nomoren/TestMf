layui.use(['layer', 'element', 'laydate', 'form'], function() {
	...
	//下载财务报表
	form.on('submit(export_finance)', function(data) {
		var data = $('#finance').serialize()
		var tokenValue=new Date().Format("yyyyMMddHHmmss");
		var downToken="down"+tokenValue;
		var url=Feng.ctxPath+'/export/exportFinance?'+data+"&downToken="+downToken+"&tokenValue="+tokenValue
